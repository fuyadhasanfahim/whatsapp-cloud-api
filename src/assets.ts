...
const specs = {
    ...
    Videos: {
        kind: "video" as const,
        mime: { ".mp4": "video/mp4", ".3gp": "video/3gpp" },
        max: 16 * 1024 * 1024,
    },
    Audios: {
        kind: "audio" as const,
...
async function readText(file: string): Promise<string> {
    ...
    if (value.length > 4096)
        throw new Error(
            `Text exceeds WhatsApp text limit (4096 characters): ${file}`,
        );
    return value;
}
...
async function mediaSteps<K extends keyof typeof specs>(
    root: string,
    folder: K,
): Promise<AssetStep[]> {
...
export async function loadAssets(root: string): Promise<AssetStep[]> {
    ...
    if (!/^https:\/\/\S+$/i.test(link) || /\s/.test(link))
        throw new Error("Assets/link.txt must contain ONE HTTPS URL");
    const extras = await listFiles(path.join(root, "Texts"), [".txt"]);
    const texts: AssetStep[] = [];
    for (const file of extras)
        texts.push({
            kind: "text",
            label: `Texts/${path.basename(file)}`,
    ...
    if (!images.length)
        console.warn("Assets/Images is empty: no images will be sent.");
    if (!videos.length)
        console.warn("Assets/Videos is empty: no videos will be sent.");
    if (!audios.length)
        console.warn("Assets/Audios is empty: no audios will be sent.");
    return [
